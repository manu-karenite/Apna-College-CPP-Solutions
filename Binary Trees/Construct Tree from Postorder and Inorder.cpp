...
BinaryTreeNode < int > * build(int * postorder, int postStart, int postEnd, int * inorder, int inStart, int inEnd) {
  ...
  // calculate the post and in of Left Subtree
  // poststart = no change
  // inStart= no change
  // inEnd = searchedIndex-1;
  // postEnd = postStart + searchedIndex-1-instart
  root -> left = build(postorder, postStart, postStart + searchedIndex - 1 - inStart, inorder, inStart, searchedIndex - 1);
  // calculate the post and in of Left Subtree
  // poststart = postStart + searchedIndex-1-instart+1
  // inStart= searchedIndex+1;
  // inEnd = no change
  // postEnd = decrease 1 
  root -> right = build(postorder, postStart + searchedIndex - 1 - inStart + 1, postEnd - 1, inorder, searchedIndex + 1, inEnd);
  return root;
...
BinaryTreeNode < int > * buildTree(int * postorder, int postLength, int * inorder, int inLength) {
  ...
  // postLength = postEnd-postStart+1;

  BinaryTreeNode < int > * root = build(postorder, postStart, postEnd, inorder, inStart, inEnd);
  return root;
}
